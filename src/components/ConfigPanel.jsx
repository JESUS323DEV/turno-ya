...
import { ChevronDown, ChevronLeft, ChevronRight, ArrowLeft, Lock, LogOut, Settings, FlaskConical, ScrollText, Store, Clock, Settings2, HelpCircle, SlidersHorizontal, Eye, Calendar, User, UserCheck, Phone, Mail, Users, MessageSquare } from "lucide-react";
...
function getFormVars(tema, colorFondo, colorAcento, colorBorde, TEMAS) {
  ...
  if (!preset) return { dataTema: "claro", vars: {} };
  const vars = {};
  ...
  return { dataTema: preset.base, vars };
}
...
export default function ConfigPanel({ config, onBack }) {
  ...
  if (cuentaOpen) {
    return (
      <section className="admin-section cfg-cuenta-page" data-tema-panel={temaPanel} style={panelVars}>
  ...
  ══════════════════════════════════════════ */
  return (
    <section className="admin-section" data-tema-panel={temaPanel} style={panelVars}>
      <div className="cfg-wrapper">
        ...
        <form className="admin-form cfg-main" onSubmit={guardar}>
          ...
          <div className="cfg-topbar">
            ...
            <h2 className="cfg-topbar-title">Configuración</h2>
            <button className="admin-btn-primary" type="submit">
              {guardado ? "✓ Guardado" : "Guardar cambios"}
            </button>
          </div>

          {/* ── Header móvil (back + avatar + menú) ── */}
          <div className="admin-header">
            ...
            <div className="cfg-mobile-profile">
              <button type="button" className="cfg-mobile-avatar-btn" onClick={() => setMenuOpen(p => !p)}>
                {draft.logoUrl
                  ? <img src={draft.logoUrl} alt="" className="cfg-mobile-avatar-img" />
                  : <User size={16} />
                }
          ...
          <div className="acc-list">
                ...
                <div className="acc-body">
                  ...
                  <div className="cfg-card">
                    <div className="cfg-card-desktop cfg-card-mobile">
                      ...
                      {/* Datos del negocio */}
                      <div className="cfg-card-cont">
                        <span className="cfg-card-subtitle">Datos del negocio</span>
            ...
            <div className="acc-seccion acc-seccion--preview">
              ...
              {abierto === "preview" && (() => {
                const { dataTema, vars } = getFormVars(draft.tema, draft.colorFondo, draft.colorAcento, draft.colorBorde, TEMAS);
                return (
                  <div className="acc-body">
                    ...
                    <div className="cfg-card">
                      <div className="cfg-card-desktop cfg-card-mobile">
                        ...
                        <div className="admin-tema-selector">
                          {TEMAS.map(({ id, label }) => {
                            const locked = id === "personalizado";
                            return (
                              <button key={id} type="button" disabled={locked}
                                className={`admin-tema-btn ${draft.tema === id ? "admin-tema-btn--active" : ""} ${locked ? "admin-tema-btn--locked" : ""}`}
                                onClick={() => {
                                  setField("tema", id);
                    ...
                    <div className="cfg-tema-preview-wrap" data-tema={dataTema} style={{ ...vars, background: "var(--bg)" }}>
          ...
          {/* Botón guardar móvil */}
          <button className="admin-btn-primary cfg-mobile-save" type="submit">
            {guardado ? "✓ Guardado" : "Guardar cambios"}
          </button>
          {errorGuardado && <p className="admin-error">{errorGuardado}</p>}
